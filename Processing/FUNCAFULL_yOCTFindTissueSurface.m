...
%                              count becomes undefined and detection quality may degrade.
%       'constantThreshold'    User-supplied intensity threshold (overrides Otsu detection). 
%                              Leave [] (default) to auto-detect.
% OUTPUTS:
...
if isstruct(dim) && isfield(dim,'x') && isfield(dim.x,'values') && ...
        numel(dim.x.values) >= 2 && isfield(dim.x,'units')
    switch lower(dim.x.units)       % unit → factor
        ...
        case {'m','meter','meters'}
            factor = 1e6;           % m  → µm
        otherwise
            factor = [];            % unknown units
    end
    if ~isempty(factor)
        pixelSize_um = abs(dim.x.values(2) - dim.x.values(1)) * factor;
    end
end
