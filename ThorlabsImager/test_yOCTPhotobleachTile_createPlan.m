classdef test_yOCTPhotobleachTile_createPlan < matlab.unittest.TestCase
    ...
    methods(Test)
        ...
        function testSurfaceCorrectionModes(tc)
                ...
                tc.verifyGreaterThan(max(dzA)-min(dzA), 0.0, ...
                ...
                tc.verifyLessThan(max(dzB)-min(dzB), 1e-6, ...
                ...
                tc.verifyLessThan(max(dzC)-min(dzC), 1e-6, ...
               ...
               % Case D: origin-tile mode with NaN at the center (reference invalid) must be zero offsets everywhere
                ...
                tc.verifyLessThan(max(dzD)-min(dzD), tol, ...
                ...
                tc.verifyLessThan(max(zD)-min(zD), tol, ...
                ...
                tc.verifyLessThan(max(dzE_uni)-min(dzE_uni), 1e-6, ...
        ...
        function testSurfaceMapAndModeCombos(tc)
            % No surfaceMap and no surfaceCorrectionMode passed must force 'none'
            % Expect zero Z offsets and unchanged stage Z (baseline z=0 by default).
            json = yOCTPhotobleachTile([-0.2;0.2],[0.2;-0.2], ...
                'skipHardware', true, ...
                'octProbePath',  tc.DummyIni, ...
                'maxLensFOV',    0.4);
        
            % Mode should be auto-forced to 'none'
            tc.verifyEqual(json.surfaceCorrectionMode, 'none', ...
                'Mode should be forced to "none" when surfaceMap is empty and mode not provided.');
        
            % Zero Z offsets and baseline stage Z
            dz = [json.photobleachPlan.zOffsetDueToTissueSurface];
            zc = [json.photobleachPlan.stageCenterZ_mm];
            tc.verifyEqual(dz, zeros(size(dz)), ...
                'Zero Z offsets expected when mode="none".');
            tc.verifyEqual(zc, zeros(size(zc)), ...
                'Stage Z should remain at the baseline when mode="none".');
        
        % Now surfaceMap but explicit per-tile mode must throw input error
            import matlab.unittest.constraints.Throws
            tc.verifyThat(@() yOCTPhotobleachTile([-0.1;0.1],[0.1;-0.1], ...
                    'skipHardware',  true, ...
                    'octProbePath',  tc.DummyIni, ...
                    'maxLensFOV',    0.4, ...
                    'surfaceCorrectionMode', 'per-tile'), ...
                Throws(?MException), ...
                'Expected an input error: surfaceMap is empty but mode="per-tile".');
        
        % Now surfaceMap but explicit origin-tile mode must also throw input error
            tc.verifyThat(@() yOCTPhotobleachTile([-0.1;0.1],[0.1;-0.1], ...
                    'skipHardware', true, ...
                    'octProbePath',  tc.DummyIni, ...
                    'maxLensFOV',    0.4, ...
                    'surfaceCorrectionMode', 'origin-tile'), ...
                Throws(?MException), ...
                'Expected an input error: surfaceMap is empty but mode="origin-tile".');
        end
    end
end
