classdef test_yOCTFindTissueSurface < matlab.unittest.TestCase
    ...
    properties (MethodSetupParameter)
        coverslip = struct(without=false , with=true);
    end
    ...
    methods(Test)
        function testDimensions(testCase)
            ...
            assert(size(x_mm,2) == 1);
            assert(size(y_mm,2) == 1);

            % Make sure x and y dimensions match surfacePosition
            assert(size(surfacePosition_mm,1) == length(y_mm))
            assert(size(surfacePosition_mm,2) == length(x_mm))

            % Make sure that size matches logMeanAbs
            assert(size(surfacePosition_mm,2) == size(testCase.logMeanAbs,2)); % x direction
            assert(size(surfacePosition_mm,1) == size(testCase.logMeanAbs,3)); % y direction
        end
        ...
        function testSurfacePositionValue(testCase)
            ...
            %% Inputs in Microns (Outputs Expected in mm)
            ...
            % Convert expectations to mm to match output
            expectedSurfacePos = expectedSurfacePos * 1e-3;
            dimX_mm = dim.x.values * 1e-3;
            dimY_mm = dim.y.values * 1e-3;

            % Get outputs (always in mm)
            [surfacePosition_mm,x_mm,y_mm] = yOCTFindTissueSurface( ...
            ...
            assert( ...
                ...
                expectedSurfacePos) ...                 Expected surface position in mm
                < 2e-3 );
            ...
            % Convert expectations to mm, same as output
            expectedSurfacePos = expectedSurfacePos * 1e-3;
            dimX_mm = dim.x.values * 1e-3;
            dimY_mm = dim.y.values * 1e-3;
        ...
        function testAssertTissueSurfaceInFocus(testCase)
            ...
            yOCTAssertTissueSurfaceIsInFocus(surfacePosition_mm-surfaceZ,x_mm,y_mm);
            
            % This should fail, move 50um out of focus, make sure that
            % function returns an error.
            testCase.verifyError(...
             @()yOCTAssertTissueSurfaceIsInFocus(surfacePosition_mm-surfaceZ+0.050,x_mm,y_mm),...
