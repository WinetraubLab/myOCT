...
% data loading). A bad vector (e.g. an over-extrapolated focus drift
% measurement) would otherwise crash deep inside the parfor loop with a
% cryptic indexing error after a long wait.
...
% (the >= 1 check ran above, before any data was loaded):
        ...
        for xxI=1:length(xCenters)
            ...
            for zzI=1:length(zDepths)
                ...
                if (isSaveSomeYPlanes && sum(yI == yToSaveI)>0)
                    ...
                    im = mag2db(scan1);
                    if ~isnan(focusPositionInImageZpix(zzI))
                        im(round(focusPositionInImageZpix(zzI)),1:20:end) = min(im(:)); % Mark focus position on sample
                    end
                    yOCT2Tif(im,tn);
                    awsCopyFile_MW1(tn, ...
